...
**Approvals pin.** Reviewers hold acknowledgment state against the proposal's head, so an approval records exactly what it endorsed, and when the proposal moves, approvals go visibly stale — the forge's "dismiss stale reviews" checkbox, as bookkeeping. Zed's Delta repairs the same failure the opposite way, by *anchoring*: the comment tracks the moving code and stays looking current. Anchoring preserves the comment's **location**; pinning preserves the reviewer's **claim** and lets the drift surface. An anchored comment on drifted code reads as endorsement of work its author never saw — the silent absorption the pattern refuses — so of the pair, only pinning needs to be a substrate guarantee. The review page — diff, out-of-date warning, mergeability, approvals — is a computed view over facts the substrate already holds (§5.3).
